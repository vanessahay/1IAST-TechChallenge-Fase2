# 1IAST - Tech Challenge Fase 2: Pipeline Dataform (`bronze_uf`, `bronze_municipio`, `bronze_meta_alfabetizacao_brasil`, `bronze_meta_alfabetizacao_uf` e `bronze_meta_alfabetizacao_municipio`)

Repositório contendo o pipeline de transformação e carga de dados no **Google Cloud Dataform** (Projeto: `vanehay`, Dataset: `1IAST_Fase2`, Repositório: `PIPE_1IAST_Fase2`), responsável por materializar as cinco tabelas particionadas da camada Bronze no BigQuery.

## 📁 Estrutura do Projeto

- `workflow_settings.yaml`: Configuração principal do repositório Dataform (Core v3.0.0, localização US).
- `definitions/sources/`: Declarações de fontes da Base dos Dados (`dicionario`, `uf`, `municipio`, diretório de UFs, diretório de Municípios, `meta_alfabetizacao_brasil`, `meta_alfabetizacao_uf` e `meta_alfabetizacao_municipio`).
