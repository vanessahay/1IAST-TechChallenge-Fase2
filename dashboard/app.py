...
st.set_page_config(
    page_title="Tech Challenge - Alfabetização & Vulnerabilidade (Gold)",
    page_icon="🎓",
    layout="wide",
...
with head_left:
    ...
    <div style='margin-bottom: 1.5rem;'>
        ...
        <p style='margin:0; font-size:0.85rem; color:var(--text-muted);'>Fase 2 - Análise de Metas, Vulnerabilidade e Predição de Defasagem (Camada Gold)</p>
    </div>
    """, unsafe_allow_html=True)
with head_right:
...
with tab2:
    st.markdown("### Busca e Detalhes por Município (Camada Gold Única)")
    
    # Filtro de UF obtido diretamente da tabela gold de indicadores municipais
    query_list_ufs = """
    SELECT DISTINCT sigla_uf 
    FROM `vanehay.1IAST_Fase2.gold_indicadores_municipais` 
    WHERE sigla_uf IS NOT NULL AND sigla_uf != 'BR'
    ORDER BY sigla_uf
    """
    ...
    # Consulta sem Joins em runtime (consumindo colunas integradas na camada Gold)
    query_mun = """
    SELECT 
...
with tab3:
    st.markdown("### Simulador de Risco de Defasagem Escolar (Modelos V2)")
